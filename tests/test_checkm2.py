...
def _concurrent_checkm2_worker(root: str, calls_dir: str, results_dir: str, errors_dir: str) -> None:
    cfg = {"CHECKM2_DATABASE_ROOT": root, "CHECKM2_AUTO_DOWNLOAD": "true"}

    def runner(command):
        Path(calls_dir, f"call-{time.time_ns()}").write_text("\t".join(command))
        time.sleep(0.2)
        _write_db(Path(command[-1]) / "CheckM2_database" / EXPECTED_CHECKM2_DB_NAME)
...
class CheckM2DatabaseTests(unittest.TestCase):
    def test_existing_shared_db_is_reused_and_recorded(self):
        with tempfile.TemporaryDirectory() as d:
            run = Path(d) / "run"; cfg = {"CHECKM2_MODE": "required", "CHECKM2_DATABASE_ROOT": str(Path(d) / "checkm2"), "CHECKM2_AUTO_DOWNLOAD": "true"}
            ...
            with patch("cleangene.workers.command_exists", return_value=True):
    ...
    def test_missing_db_auto_downloads_once(self):
        with tempfile.TemporaryDirectory() as d:
            cfg = {"CHECKM2_DATABASE_ROOT": str(Path(d) / "checkm2"), "CHECKM2_AUTO_DOWNLOAD": "true"}
            calls = []
    ...
    def test_setup_failure_stops_controller_before_preprocess(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            cfg = {"CHECKM2_MODE": "required", "CHECKM2_AUTO_DOWNLOAD": "false", "TAXONOMY_MODE": "off", "SLURM_USER_JOB_LIMIT": "2000", "SLURM_JOB_HEADROOM": "10", "SLURM_POLL_SECONDS": "0", "SLURM_MAX_PARALLEL": "100", "SLURM_ARRAY_CHUNK_SIZE": "500", "SLURM_ACCOUNT": "", "SLURM_PARTITION": "", "CHECKM2_CPUS": "1", "CHECKM2_MEM": "1G", "CHECKM2_TIME": "1:00:00"}
            atomic_json(run / "provenance" / "resolved_config.json", cfg)
            write_tsv(run / "provenance" / "manifest.tsv", ["isolate_id", "group_id", "R1", "R2"], [["i1", "g", "r1", "r2"]])
            ...
            with patch("cleangene.workers.command_exists", return_value=True), patch("cleangene.workers._run_single_job", side_effect=SystemExit("setup failed")) as single:
                with self.assertRaisesRegex(SystemExit, "setup failed"):
                    slurm_controller(run)
            self.assertTrue(any("checkm2_db_setup" in str(call) for call in single.call_args_list))
...
class CheckM2PreprocessTests(unittest.TestCase):
    ...
    def test_checkm2_crash_does_not_publish_fake_prokka_failure(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            r1.write_text("@r\n" + "A"*120 + "\n+\n" + "I"*120 + "\n"); r2.write_text(r1.read_text()); _write_db(db)
            manifest = root / "manifest.tsv"; manifest.write_text(f"isolate_id\tgroup_id\tR1\tR2\niso1\tg\t{r1}\t{r2}\n")
            cfg = {"TAXONOMY_MODE": "off", "CHECKM2_MODE": "required", "CHECKM2_DB": str(db), "READ_TRIMMING_MODE": "off", "QC_MIN_N50_PASS": "0", "QC_MIN_N50_FAIL": "0", "QC_MIN_COVERAGE_PASS": "0", "QC_MIN_COVERAGE_FAIL": "0", "PREPROCESS_USE_NODE_LOCAL_SCRATCH": "false"}
