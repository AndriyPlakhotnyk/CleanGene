...
Arrays use a rolling window rather than a full-chunk barrier. `SLURM_PREPROCESS_MAX_INFLIGHT` and `SLURM_VALIDATION_MAX_INFLIGHT` cap stage occupancy, `SLURM_MAX_PARALLEL` caps execution inside each array, and `SLURM_MAX_OUTSTANDING_CHUNKS` limits active array parents. As array elements complete, the controller refills free capacity without waiting for every element in the older array. QOS submission errors are retried after a fresh all-user occupancy check. Progress lines are reported every two minutes by default (`SLURM_CONTROLLER_REPORT_INTERVAL_SECONDS`), independently of the shorter scheduler polling interval.

Preprocessing uses `SLURM_PREPROCESS_TIME` (24 hours in the ARC template) so an unusually slow isolate does not consume the generic stage limit. If a Slurm array parent reaches `TIMEOUT`, the controller reconciles any outputs that completed before cancellation, removes the expired parent, and resubmits only isolates still missing completion markers. `SLURM_TIMEOUT_RETRIES` bounds this recovery (one retry by default); failed, cancelled, and out-of-memory states remain fatal so a real worker error is not hidden.
