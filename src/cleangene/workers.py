...
def _developer_preprocess_step(logs: Path, isolate: str, step: str, cfg: dict[str, str]):
    ...
    if enabled:
        ...
        with (logs/"developer_preprocess.tsv").open("a") as handle:
            if handle.tell()==0: handle.write("isolate_id\tstep\tstatus\telapsed_seconds\ttimestamp\tsource\n")
            handle.write(f"{isolate}\t{step}\tstarted\t\t{started:.3f}\tworker_wall_clock\n")
    failed=False
    try:
        yield
        ...
        if enabled:
            ...
            with (logs/"developer_preprocess.tsv").open("a") as handle:
                handle.write(f"{isolate}\t{step}\t{'failed' if failed else 'complete'}\t{elapsed:.3f}\t{time.time():.3f}\tworker_wall_clock\n")
...
def global_preflight(run_dir: Path) -> dict[str,object]:
    ...
    try:
        ...
        panaroo_required=assembler_mode(cfg)!="off" and any(
            not r.get("pangenome_dir","").strip()
            and (r.get("grouping_source")=="kraken_pending" or group_sizes.get(r.get("group_id",""),0)>=2)
            for r in rows
        )
        if panaroo_required:
            try:
                panaroo_version=_timed(timings,"panaroo_runtime",lambda: executable_version("panaroo","Panaroo",60))
            except ToolResolutionError as error:
                raise SystemExit(
                    "Panaroo runtime validation failed before pipeline work was submitted:\n"
                    f"{error}\n"
                    "Repair the CleanGene environment with: bash scripts/install_or_update.sh"
                ) from error
        else:
            panaroo_version="not_required"
        ...
        payload={"status":"PASS",**input_summary,"runtime":"PASS","Panaroo":panaroo_version,"CheckM2 setup":checkm2_status,"Kraken2 setup":kraken_status,"QC profiles":"PASS"}
...
def _checkm2_environment(executable: str | Path | None = None) -> dict[str,str]:
...
def compress_completed_outputs(run_dir: Path) -> dict[str,object]:
    ...
    for row in rows:
        ...
        if annotation_mode=="nonessential" and annotation_dir.is_dir():
            ...
            for path in sorted(annotation_dir.iterdir()):
                if not path.is_file() or path.is_symlink() or path.suffix==".gz" or path.suffix.lower() in {".gff",".cram",".crai"} or path.name.endswith((".archive.json",".archive.lock")): continue
...
def _successful_marker(path: Path) -> bool:
    if not _done(path): return False
    try: return str(load_json(path).get("status","")).lower()!="failed"
    except (OSError,ValueError): return False

def incomplete_indices(run_dir: Path, stage: str) -> list[int]:
    if stage=="preprocess":
        rows=read_tsv(run_dir/"state"/"isolate_tasks.tsv")
        return [i for i,r in enumerate(rows) if not _successful_marker(run_dir/"state"/"preprocess"/f"{safe_name(r['isolate_id'])}.done.json")]
    ...
    return [i for i,r in enumerate(rows) if not _successful_marker(run_dir/"state"/marker/f"{safe_name(r['group_id'])}.done.json")]
...
def _invalidate_group_after_panaroo_failure(run_dir: Path, group: str) -> None:
    group_name=safe_name(group)
    for stage in ("prepare_validation","reduce","plot"):
        _remove_done(run_dir/"state"/stage/f"{group_name}.done.json")
...
def invalidate_failed_panaroo_downstream(run_dir: Path) -> int:
    tasks=run_dir/"state"/"group_tasks.tsv"
    if not tasks.is_file(): return 0
    failed=[]
    for row in read_tsv(tasks):
        marker=run_dir/"state"/"panaroo"/f"{safe_name(row['group_id'])}.done.json"
        if marker.is_file():
            try:
                if str(load_json(marker).get("status","")).lower()=="failed": failed.append(row["group_id"])
            except (OSError,ValueError):
                continue
    ...
    for group in failed:
        ...
        for stage in ("prepare_validation","reduce","plot"):
            _remove_done(run_dir/"state"/stage/f"{group_name}.done.json")
    isolate_tasks=run_dir/"state"/"isolate_tasks.tsv"
    if isolate_tasks.is_file():
...
def run_resume_maintenance(run_dir: Path, cfg: dict[str,str]) -> dict[str,int]:
    ...
    if not (run_dir/"results/cohort/alignment_archives.tsv").is_file(): _remove_done(run_dir/"state/summary.done.json")
...
class _RollingScheduler:
    ...
    def seed_done(self,stage: str,indices: list[int]) -> set[int]:
        if stage not in self.done:
            self.done[stage]=set(); self.failed[stage]=set()
            for i in indices:
                if _index_done(self.run_dir,stage,i): self.done[stage].add(i)
                elif _index_failed(self.run_dir,stage,i): self.failed[stage].add(i)
    ...
    def submit_ready(self,stage: str,indices: list[int],cpus: str,mem: str,time_limit: str,label: str,max_inflight: int) -> int:
        ...
        while ready and avail>0 and stage_queued<max_inflight and batches<max_batches:
            ...
            self.failed.setdefault(stage,set()).difference_update(part)
...
def slurm_controller(run_dir: Path, index: int | None = None) -> None:
    with _controller_lock(run_dir):
        status_path=run_dir/"state"/"controller_status.json"
        atomic_json(status_path,{"status":"running","job_id":os.environ.get("SLURM_JOB_ID","local"),"started_at":time.time()})
        try:
            cfg, rows=context(run_dir)
            ...
            failed_groups=invalidate_failed_panaroo_downstream(run_dir)
            if failed_groups: _controller_log(f"step=panaroo_retry | invalidated_failed_group_downstream={failed_groups}",ok=True)
            ...
            _controller_log(f"controller_failed | reason={reason}")
            raise
        atomic_json(status_path,{"status":"complete","job_id":os.environ.get("SLURM_JOB_ID","local"),"completed_at":time.time()})
        _controller_log("controller_completed | status=PASS",ok=True)
...
def panaroo(run_dir: Path, index: int) -> None:
    ...
    if done.is_file():
        ...
        if status.get("status")=="failed":
            _invalidate_group_after_panaroo_failure(run_dir,group)
            done.unlink()
    ...
    try:
        ...
        raise RuntimeError(f"Panaroo failed for group {group}: {reason}\n{stderr_tail or stdout_tail}") from error
...
def validate(run_dir: Path, index: int) -> None:
    ...
    rr=retained[iso]; validate_isolate(ref,key,locus_file,Path(rr["assembly"]),rr["R1"],rr["R2"],ev,int(cfg["VALIDATION_CPUS"]),float(cfg["READ_VALIDATION_MIN_BREADTH"]),float(cfg["READ_VALIDATION_MIN_MEAN_DEPTH"]),float(cfg["READ_VALIDATION_MIN_IDENTITY"]),int(cfg["READ_VALIDATION_MIN_MAPQ"]),int(cfg["BASEQUAL"]),initial_calls=initial,truncation_breadth=float(cfg["READ_VALIDATION_TRUNCATION_MIN_BREADTH"]),divergent_breadth=float(cfg["READ_VALIDATION_DIVERGENT_MIN_BREADTH"]),divergent_identity=float(cfg["READ_VALIDATION_DIVERGENT_MIN_IDENTITY"]))
    metric_rows=read_tsv(ev/"metrics.tsv")
    if not any(r.get("arbitration_status")=="pending" for r in metric_rows):
        for name in ("discovered_genes.fasta","discovered_genes.tsv"):
            (ev/name).unlink(missing_ok=True)
        shutil.copy2(ev/"metrics.tsv",ev/"arbitrated_metrics.tsv")
        archive_own_alignment(ev,int(cfg["VALIDATION_CPUS"]))
        touch_done(run_dir/"state"/"arbitrate"/f"{safe_name(iso)}.done.json",{"status":"not_required","discordant_cases":0})
    touch_done(done)
...
def arbitrate_evidence(row: dict[str,str], *, deletion_spanned: bool=False) -> dict[str,str]:
    ...
    elif row.get("evidence_state") in {"not_detected", "confirmed_present"}:
        result.update(validated_call="0" if row["evidence_state"] == "not_detected" else "1", arbitration_status="not_required", arbitration_reason="terminal read evidence", final_call_source="read_validation")
    elif row.get("evidence_state") in {"possible_truncation"} and row.get("initial_call")=="1":
...
def arbitrate(run_dir: Path,index: int) -> None:
    ...
    initial_matrix=read_tsv(root/"02_pangenome"/"initial_calls"/"gene_presence_absence.binary.tsv")
    present_count=sum(int(r[iso]) for r in initial_matrix)
    maximum,pending=arbitration_cases(metrics,present_count,float(cfg["READ_VALIDATION_ARBITRATION_FRACTION"]))
    selected={r["Gene"] for r in pending[:maximum]}; sample=retained_rows(run_dir,row["group_id"]); sample=next((r for r in sample if r["isolate_id"]==iso),{}); assembly=Path(sample["assembly"]) if sample.get("assembly") else None; assembly_seqs=read_fasta(assembly) if assembly and assembly.is_file() else {}; ref_path=root/"03_read_validation"/"tested_gene_references.fasta"; refs=read_fasta(ref_path) if ref_path.is_file() else {}; results=[]; discoveries=[]
    pending_rank={r["Gene"]:rank for rank,r in enumerate(pending)}
    for metric in sorted(metrics,key=lambda r:pending_rank.get(r["Gene"],len(pending))):
        if metric.get("arbitration_status")!="pending" or metric.get("evidence_state") in {"confirmed_present","not_detected"}: results.append(metric); continue
        ...
        if bam.name=="own_assembly_reads.bam" and not bam.is_file() and (ev/"own_assembly_reads.archive.json").is_file(): restore_own_bam(ev,bam,int(cfg["ARBITRATION_CPUS"]))
        if bam.name=="pangenome_reads.bam" and not bam.is_file():
            from .final_archives import archive_paths, restore_bam
            manifest=archive_paths(bam)[1]
            if manifest.is_file(): restore_bam(manifest,bam,int(cfg["ARBITRATION_CPUS"]))
        ...
        try:
            discovery=discover_cds(reconstruction,metric,ev/"arbitration"/safe_name(metric["Gene"]),int(cfg["ARBITRATION_CPUS"]),float(cfg["READ_VALIDATION_MIN_MEAN_DEPTH"]),int(cfg["READ_VALIDATION_MIN_MAPQ"]),int(cfg["BASEQUAL"]))
        except (OSError,subprocess.CalledProcessError) as error:
            results.append({**arbitrate_evidence(metric),"arbitration_reason":f"CDS reconstruction evaluation failed: {error}"}); continue
        if discovery: discoveries.append({**discovery,"parent_read_breadth":metric.get("read_breadth") or metric.get("breadth") or 0,"parent_min_breadth":cfg["READ_VALIDATION_MIN_BREADTH"]})
        ...
        if match and metric.get("evidence_state")=="partial_homolog" and float(metric.get("read_breadth") or metric.get("breadth") or 0)<float(cfg["READ_VALIDATION_MIN_BREADTH"]):
            results.append({**metric,"validated_call":"0","final_call_source":"partial_homolog_read_gate","arbitration_status":"resolved","arbitration_reason":"distinct CDS discovery does not establish original gene presence below required read breadth","reconstructed_coverage":match["breadth"]})
            continue
        if match:
            from .evidence import orf_integrity
            candidate_seq=read_fasta(Path(reconstruction["contigs_path"]))[match["contig"]][match["query_start"]:match["query_end"]]
            if match["strand"]=="-": candidate_seq=candidate_seq.translate(str.maketrans("ACGTN","TGCAN"))[::-1]
    ...
    write_fasta(ev/"discovered_genes.fasta",[(r["candidate_id"],r["sequence"]) for r in discoveries])
    write_tsv(ev/"discovered_genes.tsv",["candidate_id","parent_gene","discovery_reason","parent_read_breadth","parent_min_breadth","sequence_sha256","breadth","identity","mean_depth","mapped_reads","orf_integrity"],discoveries)
    write_tsv(ev/"arbitration_cases.tsv",METRIC_FIELDS,pending); write_tsv(ev/"arbitrated_metrics.tsv",METRIC_FIELDS,results)
    archive_own_alignment(ev,int(cfg["ARBITRATION_CPUS"]))
    touch_done(done,{"discordant_cases":len(pending),"panaroo_present_genes":present_count,"arbitration_cap":maximum,"processed_cases":min(len(pending),maximum),"deferred_cases":max(0,len(pending)-maximum)})
...
def reduce_group(run_dir: Path, index: int) -> None:
    ...
    for iso in isolates:
        ...
        for r in read_tsv(p) if p.is_file() else []:
            ...
            metrics.append(r)
    reference_path=out/"tested_gene_references.fasta"; key_path=out/"tested_gene_key.tsv"
    all_sequences=read_fasta(reference_path) if reference_path.is_file() else {}
    keys=read_tsv(key_path) if key_path.is_file() else []
    existing={r["Gene"]:all_sequences[r["reference_id"]] for r in keys if r["reference_id"] in all_sequences}
    missing_references=[r for r in initial if r["Gene"] not in existing]
    if missing_references and prepared_pangenome_dir(run_dir,group,root).is_dir():
        recovered,recovered_sources=recover_sequences(missing_references,prepared_pangenome_dir(run_dir,group,root))
        recovered=dict(recovered)
        existing.update({r[1]:recovered[r[0]] for r in recovered_sources})
    new_sequences,new_calls,new_sources=consolidate_discoveries(out,isolates,existing,int(cfg["VALIDATION_CPUS"]),float(cfg["NOVEL_GENE_CLUSTER_IDENTITY"]),float(cfg["NOVEL_GENE_CLUSTER_COVERAGE"]))
    for gene in new_sequences: by_gene[gene]={iso:0 for iso in isolates}
    ...
    blocked_partial={(r["Gene"],r["isolate_id"]) for r in metrics if r.get("evidence_state")=="partial_homolog" and float(r.get("read_breadth") or r.get("breadth") or 0)<float(cfg["READ_VALIDATION_MIN_BREADTH"])}
    for gene,iso in blocked_partial: validated[gene][iso]=0
    for metric in metrics:
        if (metric["Gene"],metric["isolate_id"]) in blocked_partial: metric["validated_call"]=0
    for gene,calls in new_calls.items():
        for iso,call in calls.items():
            if call and (gene,iso) not in blocked_partial: validated[gene][iso]=1
    source_index={(r["Gene"],r["isolate_id"]):r for r in new_sources if (r["Gene"],r["isolate_id"]) not in blocked_partial}
    metrics=[r for r in metrics if (r["Gene"],r["isolate_id"]) not in source_index]
    for (gene,iso),source in source_index.items():
        metrics.append({**source,"Gene":gene,"isolate_id":iso,"initial_call":by_gene[gene][iso],"validated_call":1,"evidence_version":EVIDENCE_VERSION,"evidence_state":"discovered_complete_cds","validation_state":"discovered_complete_cds","decision_reason":"reconstructed complete CDS","decision_metrics":"breadth;identity;mean_depth;orf_integrity","final_call_source":"consolidated_discovery","sequence_resolution":"reconstructed","arbitration_status":"resolved","arbitration_reason":"complete CDS with direct recruited-read support"})
    for gene in new_sequences:
        for iso in isolates:
            if (gene,iso) not in source_index:
                metrics.append({"Gene":gene,"isolate_id":iso,"initial_call":0,"validated_call":"","evidence_version":EVIDENCE_VERSION,"evidence_state":"new_gene_not_tested","validation_state":"new_gene_not_tested","decision_reason":"new gene not tested in isolate","final_call_source":"initial_call_unresolved","sequence_resolution":"unresolved"})
    augmented_initial=out/"initial_calls.with_discovered_genes.tsv"
    write_tsv(augmented_initial,["Gene",*isolates],([g,*[by_gene[g][i] for i in isolates]] for g in by_gene))
    ...
    write_validation_summary(augmented_initial,validated_matrix,out/"gene_call_evidence.long.tsv",out,group)
