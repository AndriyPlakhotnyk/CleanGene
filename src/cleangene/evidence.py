...
def map_reads(reference: Path,r1: str,r2: str,bam: Path,threads: int,min_mapq: int,log: Path,*,retain_ambiguous: bool=False) -> None:
    ...
    archive_manifest=archive_paths(bam)[1]
    if not bam.is_file() and archive_manifest.is_file(): restore_bam(archive_manifest,bam,threads)
...
def validation_decision_logic_rows(min_breadth="0.95",min_depth="5",min_identity="0.95") -> list[list[str]]:
    return [
        ...
        ["partial_homolog","partial coverage or weak similarity",f"0 unless original read breadth >= {min_breadth} and further evidence resolves presence","Related sequence; discovery may identify a distinct CDS without promoting the parent"],
